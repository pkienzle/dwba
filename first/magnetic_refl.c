...
cdouble rhoN(global const double *L, int j);
cdouble u1(global const double *L, int j);
cdouble u3(global const double *L, int j);
double rhoB(global const double *L, int j);
double dz(global const double *L, int j);
cdouble rhoN(global const double *L, int j) { return cplx(L[j*8],L[j*8+1]); }
cdouble u1(global const double *L, int j)   { return cplx(L[j*8+2],L[j*8+3]); }
cdouble u3(global const double *L, int j)   { return cplx(L[j*8+4],L[j*8+5]); }
double rhoB(global const double *L, int j) { return L[j*8+6]; }
double dz(global const double *L, int j)   { return L[j*8+7]; }
...
*/
    ...
    // E0 = kzi**2 + 4*pi*rhoN + 4*pi*rhoB
    const cdouble E0 = one*(kzi*kzi+spin*PI4*rhoB(L,surface)) + PI4*rhoN(L,surface);

    U1n = u1(L,1);
    U3n = u3(L,1);
    S1p = -csqrt(PI4*(rhoN(L,0)+one*rhoB(L,0)) - E0 - I*EPSILON);
    S3p = -csqrt(PI4*(rhoN(L,0)-one*rhoB(L,0)) - E0 - I*EPSILON);
    S1n = -csqrt(PI4*(rhoN(L,1)+one*rhoB(L,1)) - E0 - I*EPSILON);
    S3n = -csqrt(PI4*(rhoN(L,1)-one*rhoB(L,1)) - E0 - I*EPSILON);
    ...
    // First B matrix is slightly different
    {
        ...
        B43 = B34;
        B44 = B33;
        // std::cout << "E0=" << E0 << " U1n=" << U1n << " S1p="<<S1p<<" FS11="<<FS11<<std::endl;
    }
...
#elif 0
std::cout<<"===== B ===="<<std::endl;
std::cout<<"kzi="<<kzi<<" layer="<<(i-1)<<" B11="<<B11<<std::endl;
std::cout<<"["<<B11<<" "<<B12<<" "<<B13<<" "<<B14<<"]"<<std::endl;
std::cout<<"["<<B21<<" "<<B22<<" "<<B23<<" "<<B24<<"]"<<std::endl;
std::cout<<"["<<B31<<" "<<B32<<" "<<B33<<" "<<B34<<"]"<<std::endl;
std::cout<<"["<<B41<<" "<<B42<<" "<<B43<<" "<<B44<<"]"<<std::endl;
#endif
        ...
        U1n = u1(L,i);
        U3n = u3(L,i);
        S1n = -csqrt(PI4*(rhoN(L,i)+one*rhoB(L,i))-E0 - I*EPSILON);
        S3n = -csqrt(PI4*(rhoN(L,i)-one*rhoB(L,i))-E0 - I*EPSILON);
...
#ifdef USE_OPENCL
    //printf("> offset:%d kz:%d Nlayers:%d kz[0]:%g\n",kz_offset,Nkz,Nlayers,kz[0]);
    //const int Kstride = 1;
